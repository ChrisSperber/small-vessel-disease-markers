...
NMF_SINGLE_LATENT = "NMF1of1"
PCA_FIRST_LATENT = "PCA1"
...
ADDITIONAL_PROSPECTIVE_DATA_CSV = DATA_FOLDER / "additional_prospective_data.csv"
...
prospective_data_df = pd.read_csv(PROSPECTIVE_SAMPLE_CLEAN_CSV, sep=";")

prospective_latents_arr = compute_svd_latent(
    df=prospective_data_df,
    model_weights_path=MODEL_WEIGHTS,
    preproc_json_path=PREPROC_JSON,
)

prospective_data_df[LATENT_SVD_SCORE] = prospective_latents_arr

# load and merge simple latents for prospective data
simple_latents_df = pd.read_csv(SIMPLE_LATENTS_CSV, sep=",")
prospective_data_df = prospective_data_df.merge(
    simple_latents_df, how="inner", on=[Cols.ID, Cols.INSEL_PID], validate="1:1"
)

# %%
# load additional prospective data with clinical scores & merge
additional_prosepctive_data_df = pd.read_csv(ADDITIONAL_PROSPECTIVE_DATA_CSV, sep=";")
prospective_data_df = prospective_data_df.merge(
    additional_prosepctive_data_df, how="inner", on=Cols.ID, validate="1:1"
)
...
y_bin = prospective_data_df[Cols.NIHSS_24H_BINARY_GT4].astype(int).to_numpy()
y_cont = prospective_data_df[Cols.NIHSS_24H].to_numpy(dtype=float)

lines.append("\n\nSVD autoencoder evaluation\n")
lines.append("External validation/prospective data\n")
lines.append(f"N = {len(prospective_data_df)}\n")

# --- 1) Logistic regression + AUC ---
lines.append("\nLogistic regression: NIHSS binary ~ SVD score (univariate)\n")
for score_col in svd_scores:
    x = prospective_data_df[[score_col]].to_numpy(dtype=float)
